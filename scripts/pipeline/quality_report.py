...
def main() -> int:
    ...
    write_csv(report_dir / "coverage_by_year.csv", coverage_by_year(rows))
    write_csv(report_dir / "missing_dates.csv", missing_dates(rows, args.from_date, args.to_date))
    write_csv(report_dir / "price_outlier_report.csv", price_outliers(rows))
    write_csv(report_dir / "current_leakage_report.csv", current_leakage(rows))
...
def coverage_by_year(rows: list[dict[str, str]]) -> list[dict[str, object]]:
...
def missing_dates(rows: list[dict[str, str]], start: str, end: str) -> list[dict[str, str]]:
    present = {row.get("date") for row in rows}
    return [{"date": value, "issue": "missing_domestic_gold_quote"} for value in date_range(start, end) if value not in present]


def price_outliers(rows: list[dict[str, str]]) -> list[dict[str, object]]:
    values = []
    for row in rows:
        try:
...
def current_leakage(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    output = []
    for row in rows:
        if row.get("business_date") and row.get("date") and row["business_date"] != row["date"]:
            output.append({"date": row["date"], "business_date": row["business_date"], "source": row.get("source", ""), "issue": "business_date_mismatch"})
    return output
